...
const ListSocial = [
    {
        ...
        icon: (
            <svg
                ...
                class="lucide lucide-linkedin w-6 h-6 transition-all duration-500 group-hover:scale-105"
        ...
        sizeSubName: 'text-sm',
        bg: '#0a66c2',
        col: 'lg:col-span-2 col-span-1',
    },

    {
        name: 'Github',
        subName: '@longdev09',
        url: 'https://github.com/longdev09',
        icon: (
            <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
...

    {
        name: 'Facebook',
        subName: '@long.dev.09',
        url: 'https://www.facebook.com/long.dev.09',
        icon: (
            <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
                ...
                stroke-linecap="round"
                stroke-linejoin="round"
                class="lucide lucide-facebook w-6 h-6 transition-all duration-500 group-hover:scale-105"
                style={{ color: 'rgb(10, 102, 194)' }}
            >
    ...
    },
    {
        name: 'Instagram',
        subName: '@long.3902',
        url: 'https://www.instagram.com/long.3902/',
        icon: (
            <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
                ...
                stroke-linecap="round"
                stroke-linejoin="round"
                class="lucide lucide-instagram w-5 h-5 transition-all duration-500 group-hover:scale-110"
        ...
        gradient: 'from-[#833AB4] to-[#FCAF45]',
        sizeName: 'text-base',
        sizeSubName: 'text-sm',
        bg: '#2c050c',
    },

    {
        name: 'TikTok',
        subName: '@nongdev',
        url: 'https://www.tiktok.com/@nong_dev',
        icon: (
            <svg width="24px" height="24px" viewBox="0 0 45 45" version="1.1" xmlns="http://www.w3.org/2000/svg">
                <title>Tiktok</title>
        ...
        <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-4">
            {ListSocial.map((item, index) => (
                <a
                    ...
                    target="_blank"
                    key={index}
                    className={` border border-[#ffffff1a] ${item.col} cursor-pointer group bg-white/5 transition-all relative inset-0 duration-500 p-4 rounded-lg flex items-center justify-between`}
                >
                    <div
                        className={`rounded-lg absolute inset-0 opacity-0 group-hover:opacity-20 transition-opacity duration-500
                    ...
                    <div className="flex flex-row items-center gap-3">
                        <div className="p-2  relative inset-0 z-40">
                            <div
                                className={`absolute inset-0 bg-[${item.bg}]  opacity-20 rounded-lg`}
                                style={{ backgroundColor: item.bg }}
                            ></div>
                            {item.icon}
                        </div>
                        <div className="flex flex-col ">
                            <span className={` ${item.sizeName} text-white font-bold`}>{item.name}</span>
                            <span className={`${item.sizeSubName} text-gray-400 text-sm`}>{item.subName}</span>
                        </div>
                    </div>

                    <span className={`${item.sizeName} text-white opacity-0 duration-500 group-hover:opacity-100`}>
